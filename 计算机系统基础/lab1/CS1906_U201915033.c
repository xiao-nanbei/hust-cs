...
int byteNot(int x, int n) {
    /*
     * （0xff<<(n<<3))即ff向左移动n个字节
     * 进行^运算,可以保证ff移位后的数字中,为1的位让x的相应的位取反,而为0的位让x相应的位不变
     */
  return x ^ (0xff << (n << 3));
}
/* 
...
int byteXor(int x, int y, int n) {
    /*
     * x >> (n<<3)和y >> (n<<3)可以把对应的字节的数据移动到最左端
     * 通过&0xff将0字节以外的数据都置0，再进行异或操作
     * !!转换为逻辑的0或1
     */
...
int rotateLeft(int x, int n) {
    /*
     * 把这个分成两部分加起来，即移位后溢出的一部分作为低位加到原数上
     */
    int m;
    m = ~(((1 << 31) >> 31) << n);
    x = ((x >> (32 + (~n + 1))) & m) + (x << n);
...
int mul2OK(int x) {
    /*
     * 第二个移位操作本应该是移动31位，但我的电脑是64位，只能通过这样来通过测试集
     */
    int m = ((x>>31)&1)^((x>>32)&1);
    return m^ 1;
}
