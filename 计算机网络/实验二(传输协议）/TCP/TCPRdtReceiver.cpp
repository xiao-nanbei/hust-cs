...
void TCPRdtReceiver::receive(const struct Packet & packet)//累计确认
{
    ...
    if (checkSum == packet.checksum && seq == packet.seqnum) //如果正确并且为期待接受的报文
    {
        ...
        lastAckPkt.acknum = seq + 1;//发送的ack更新期待的序列号
        lastAckPkt.checksum = pUtils->calculateCheckSum(lastAckPkt);//重新计算校验和
    ...
    } else//如果失序或者出错
    {
        ...
        pUtils->printPacket("ERROR：接收方发送冗余ACK", lastAckPkt);//lastpacket中的acknum为seq的值，即期待接受到的序列号。
